...
// Inject and wire related section
  ...
  const postURL = `/api/v1/news-posts/${newsId}`;
  const readNextURL = `/api/v1/news/${newsId}/read-next`;
  const mostReadNewsURL = `/api/v1/analytics/most-read/news`;
  const relatedURL = `/api/v1/news/${newsId}/related`;
  ...
  (async () => {
    try {
      ...
      if (!postRes.ok) {
        ...
        console.log('[news] post payload:', data);
        post = data;
      }
      ...
      if (!readNextRes.ok) {
        ...
        // after Promise.all([... mostReadNewsURL ...]) resolves:
        let mostReadItems = [];
        if (!mostReadRes.ok) {
